...
async function handleCheckoutSessionCompleted(supabaseClient: any, event: any) {
  ...
  try {
    ...
    const updatedStripeSubscription = await stripe.subscriptions.update(
      subscriptionId,
      {
        metadata: {
          ...session.metadata,
          checkoutSessionId: session.id,
          redirectToDashboard: "true", // Add flag to indicate redirect to dashboard
        },
      },
    );
    ...
    // Update user's subscription status in users table if userId exists
    if (userId) {
    ...
    return new Response(
      JSON.stringify({
        ...
        subscriptionId,
        redirectUrl: "/dashboard", // Include redirect URL in response
      }),
      {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
